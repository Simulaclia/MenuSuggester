...
public class RandomSuggestPageController {
	...
	void initialize() {
		Label[] Label1 = { ciusinename, genre, country, time, cal, pro, lip, car, foodstuff1, foodstuff2, foodstuff3 };

		randomRecipe = new ArrayList<ArrayList<String>>();
		// 絞り込み条件取得
		StrNarrowData = RandomNarrowdownController.getStrNarrowDate();
		//.javaの方の処理をここで取ってくる感じ
	...
	void RandomSetMenu(ArrayList<ArrayList<String>> RandomSuggestMenu, Label[] label) {
		for (int i = 0; i <= 1; i++) {
			...
			label[13].setText(RandomSuggestMenu.get(randomValue).get(13)); // 料理名検索リンク

			randomRecipe.add(RandomSuggestMenu.get(randomValue));
